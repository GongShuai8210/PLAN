...
from nets.plam_domain_net import *
...
import pickle
import torch
# def count_flops(model, image,ratios=None):
#     with torch.no_grad():
#         original_state = {name: layer.training for name, layer in model.named_children()}
#
#         # 禁用不参与训练的层
#         for name, layer in model.named_children():
#             if name not in ['image_encoder', 'text_encoder']:
#                 layer.eval()
#
#
#         # 禁用所有层，除了特定的层
...
# losslogdir = "VPT9_deep_loss"
# writer = SummaryWriter(losslogdir)
...
def train_maple(a_iter, wi, args, model, data_loader, optimizer, scheduler, device,communication_i):
    model.train()
    batch_id = 0
        ...
        for key in server_model.state_dict().keys():
            ...
            if 'prompt_learner' in key or "UnifiedPrompt" in key:
                for client_idx in range(client_num):
                    # 判断prompt_learner里的是VPT还是adaptertext
                    # adapter 共享参数
...
if __name__ == '__main__':
    ...
    # save_model = torch.load(save_path + "POSE1.pth")
    # with torch.no_grad():
    #     for key in server_model.state_dict().keys():
    #         if key in save_model.keys():
    #             server_model.state_dict()[key] = save_model[key]


    server_model = server_model.to(device=device)

    # test_acc_list = [0. for j in range(4)]
    ...
    # server_model = torch.compile(server_model, mode="max-autotune")
    # models =  [torch.compile(model, mode="max-autotune") for model in models]
    ...
    for a_iter in range(args.iters):
        ...
        # optimizers = [optim.AdamW(params=[{'params': models[idx].parameters()}], lr=args.lr2, betas=(
        #     args.beta1, args.beta2), eps=args.eps, weight_decay=args.weight_decay) for idx in range(client_num)]




        schedulerss = [0] * client_num
        ...
        for wi in range(wk_iters):
            ...
            for client_idx, model in enumerate(models):
                ...
                if client_idx in args.test_envs:
                    ...
                    # schedulers[client_idx].step()  # 使用余弦退火学习率调度器
                    # # 如果有 ConstantWarmupScheduler，也更新它
                    # schedulerss[client_idx].step()  # 使用常数预热学习率调度器
                    num_models += 1
                    # 使用相应的优化器进行本地训练
        ...
        with torch.no_grad():
            ...
            if best_changed:
                ...
                torch.save(server_model, 'POSE_domainnet' + str(args.test_envs[0]) + '.pth')
